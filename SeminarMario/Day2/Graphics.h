...
public:
	LivesGraphics() 
		:_singleLife(Frame(R"(../Animations/heart.png)")), _livesCount(4)
	{
		resize(_singleLife.image, _singleLife.image, Size(40, 30));
		resize(_singleLife.mask, _singleLife.mask, Size(40, 30));
	}

public:
	virtual cv::Mat getCollisionMask();
	virtual void draw(cv::Mat& canvas, cv::Point const& topLeft = Point(1300, 20));
...
public:
	...
	virtual void reset(int code = 1) override;
	virtual bool update();
};
...
public:
	AppleGraphics(/*AnimationPtr animPtr*/ Frame singalApple)
		:_singalApple(singalApple)
	{
		resize(_singalApple.image, _singalApple.image, Size(55.5, 80.5));
		resize(_singalApple.mask, _singalApple.mask, Size(55.5, 80.5));
...
class BlocksGraphics : public IGraphicsComponent
{
protected:
	Frame _singleBlock; 
	//int _livesCount;
...
public:
	BlocksGraphics()
		:_singleBlock(Frame(R"(../Animations/wall.png)"))
	{
		//resize(_singleBlock.image, _singleBlock.image, Size(40, 30));
		//resize(_singleBlock.mask, _singleBlock.mask, Size(40, 30));
	}
